  ...
  const handleSave = async () => {
    ...
    try {
      const result = await storage.saveWord({
        word: pending.word, sentence: pending.sentence, sourceUrl: pending.sourceUrl,
    ...
    <div className="mb-4 rounded-xl border-2 border-accent/30 bg-gradient-to-br from-accent-soft/60 to-white p-4 shadow-sm space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-accent text-[10px] font-bold text-white">+</span>
          <h2 className="font-display text-xl font-bold text-ink-900">{pending.word}</h2>
        </div>
        <span className="text-xs text-ink-400">Từ mới</span>
      </div>
      ...
      <div>
        ...
        <input ref={meaningRef} type="text" className="field" placeholder="Nhập hoặc chỉnh sửa nghĩa…"
...
function WordDetail({ vocab, encounters, onUpdated, onDeleted }: {
  ...
  useEffect(() => {
    setMeaning(vocab.meaning ?? ''); setExample(vocab.example ?? ''); setNote(vocab.note ?? '');
    setStatus(null); setDictEntry(vocab.dictionary ?? null); setDictLoading(!vocab.dictionary);
  }, [vocab.id, vocab.meaning, vocab.example, vocab.note, vocab.dictionary]);
  ...
  return (
    <section className="mb-4 card-soft space-y-3">
      <div className="flex items-start justify-between gap-2">
        <h2 className="font-display text-2xl font-semibold text-ink-900">{vocab.word}</h2>
      ...
      <div>
        ...
        <input type="text" className="field" placeholder="Nhập nghĩa…" value={meaning}
      ...
      <div>
        <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-ink-600">Ghi chú</label>
        <textarea className="field min-h-[50px] resize-y" placeholder="Ghi chú cá nhân…" value={note}
...
function SettingsTab() {
  ...
  const onPull = async () => {
    ...
    try {
      ...
      const { imported, updated } = await storage.importCSV(
        ['Word,Meaning,Example,Note,Encounter Count,Next Review,First Seen,Last Seen',
          ...rows.map((r) => [
            r.Word ?? '', r.Meaning ?? '', r.Example ?? '', r.Note ?? '',
            r['Encounter Count'] ?? '', '', r['First Seen'] ?? '', r['Last Seen'] ?? '',
  ...
  return (
    <div className="space-y-5">
      {/* ☁️ Google Sheets Sync — ĐẶT TRÊN CÙNG */}
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-600">☁️ Google Sheets Sync</h3>
        ...
        {sheetsLoading ? (
          ...
          /* Chưa cấu hình client_id */
          <div className="rounded-lg border border-dashed border-orange-300 bg-orange-50 p-3 space-y-2">
            <p className="text-xs font-semibold text-orange-800">⚠️ Chưa cấu hình Google OAuth</p>
            <p className="text-[11px] text-orange-700">
              Developer cần tạo Google Cloud project và thêm client_id vào file{' '}
              <code className="rounded bg-orange-100 px-1 py-0.5 font-mono text-[10px]">src/config/google.ts</code>
            </p>
            <p className="text-[11px] text-orange-600">Xem tab ❓ Hướng dẫn để biết chi tiết.</p>
          </div>
        ) : !signedIn ? (
          /* Chưa đăng nhập */
          <div className="rounded-lg border border-ink-200 bg-white p-3 space-y-3">
            <p className="text-xs text-ink-700">Đồng bộ từ vựng với Google Sheets để backup an toàn trên cloud.</p>
            <button type="button" className="btn-primary w-full flex items-center justify-center gap-2"
              ...
              {sheetsAction || 'Đăng nhập Google'}
            </button>
          </div>
        ) : !sheetsConfig?.spreadsheetId ? (
          /* Đã đăng nhập, chưa có sheet */
          <div className="rounded-lg border border-accent/20 bg-white p-3 space-y-3">
            <div className="flex items-center gap-2 text-xs text-accent-dark">
              ...
              {sheetsConfig?.userEmail && <span className="text-ink-400">({sheetsConfig.userEmail})</span>}
            </div>

            <button type="button" className="btn-primary w-full" onClick={() => void onCreateSheet()} disabled={!!sheetsAction}>
              {sheetsAction || '📝 Tạo Spreadsheet mới'}
            </button>

            <div className="flex items-center gap-2 text-[11px] text-ink-400">
              <div className="h-px flex-1 bg-ink-100" /> hoặc <div className="h-px flex-1 bg-ink-100" />
            </div>

            <div className="space-y-1.5">
              <label className="block text-[11px] font-medium text-ink-600">Kết nối sheet có sẵn (nhập ID):</label>
              <input type="text" className="field text-xs" placeholder="Spreadsheet ID…"
                value={connectId} onChange={(e) => setConnectId(e.target.value)}
                onKeyDown={(e) => { if (e.key === 'Enter') void onConnectSheet(); }} />
              <button type="button" className="btn-ghost w-full border border-ink-200 text-xs"
                onClick={() => void onConnectSheet()} disabled={!!sheetsAction || !connectId.trim()}>
                Kết nối
              </button>
              <p className="text-[10px] text-ink-400">
                ID nằm trong URL: docs.google.com/spreadsheets/d/<strong>ID_Ở_ĐÂY</strong>/edit
              </p>
            </div>

            <button type="button" className="text-[11px] text-ink-400 hover:text-ink-600 underline"
          ...
          <div className="rounded-lg border border-accent/20 bg-white p-3 space-y-3">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="font-medium text-ink-900">Đã kết nối</span>
            </div>

            <div className="rounded-md bg-ink-50 px-3 py-2 text-xs">
              <p className="font-medium text-ink-800 truncate">📊 {sheetsConfig.spreadsheetTitle || 'Spreadsheet'}</p>
              <a href={sheets.getSheetUrl(sheetsConfig.spreadsheetId)}
                target="_blank" rel="noopener noreferrer"
                className="text-accent hover:underline text-[11px] mt-0.5 inline-block">
                Mở Google Sheet ↗
              </a>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <button type="button" className="btn-primary text-xs py-2 flex items-center justify-center gap-1"
                onClick={() => void onPush()} disabled={!!sheetsAction}>
                ⬆ Push lên Sheet
              </button>
              <button type="button" className="btn-ghost border border-ink-200 text-xs py-2 flex items-center justify-center gap-1"
            ...
            {sheetsAction && (
              <div className="flex items-center gap-2 text-xs text-ink-500">
                <svg className="h-3 w-3 animate-spin text-accent" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <circle cx="12" cy="12" r="10" strokeOpacity=".25"/>
                  <path d="M12 2a10 10 0 0 1 10 10" strokeLinecap="round"/>
                </svg>
                {sheetsAction}
      ...
      {/* Excel */}
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-600">📊 Excel (.xlsx)</h3>
        <div className="space-y-2">
          <button type="button" id="export-xlsx" className="btn-primary w-full" onClick={() => void onExportExcel()}>
            Xuất Excel (.xlsx)
          </button>
          <button type="button" id="import-xlsx" className="btn-ghost w-full border border-ink-200"
      ...
      {/* CSV */}
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-600">📄 CSV</h3>
        <div className="space-y-2">
          <button type="button" id="export-csv" className="btn-primary w-full" onClick={() => void onExportCSV()}>
            Xuất CSV
          </button>
          <button type="button" id="import-csv" className="btn-ghost w-full border border-ink-200"
        ...
        <p className="mt-1.5 text-[11px] text-ink-400">
          Cột: Word, Meaning, Example, Note, Encounter Count, Next Review, First Seen, Last Seen
        </p>
      </div>

      {/* JSON backup */}
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-600">💾 Backup JSON</h3>
        <div className="space-y-2">
          <button type="button" className="btn-ghost w-full border border-ink-200"
            onClick={() => void (async () => {
      ...
      {srsConfig && (
        <div>
          <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-600">🧠 Ôn tập (Spaced Repetition)</h3>
          <div className="rounded-lg border border-ink-200 bg-white p-3 space-y-3">
            <p className="text-[11px] text-ink-500">Cài đặt khoảng cách (ngày) cho lần ôn đầu tiên theo mức độ nhớ:</p>
            <div className="grid grid-cols-4 gap-2">
              {([0, 1, 2, 3] as ReviewGrade[]).map((grade) => {
                const g = GRADE_LABELS[grade];
                const colorBg = grade === 0 ? 'bg-red-50 border-red-200'
                  : grade === 1 ? 'bg-orange-50 border-orange-200'
                  : grade === 2 ? 'bg-emerald-50 border-emerald-200'
                  : 'bg-sky-50 border-sky-200';
                return (
                  <div key={grade} className={`rounded-lg border p-2 text-center ${colorBg}`}>
                    <p className="text-[10px] font-semibold mb-1">{g.label}</p>
                    <input type="number" min={0} max={365}
                      className="w-full rounded border border-ink-200 bg-white px-1.5 py-1 text-center text-xs"
                    ...
                    <p className="text-[9px] text-ink-400 mt-0.5">ngày</p>
                  </div>
                );
              })}
            </div>
            <div className="flex items-center gap-2">
              ...
              <input type="number" min={1} max={365}
                className="w-16 rounded border border-ink-200 px-1.5 py-1 text-center text-xs"
              ...
              <span className="text-[11px] text-ink-400">ngày</span>
            </div>
            <p className="text-[10px] text-ink-400">
              Lần ôn thứ 3+ sẽ tự tính theo thuật toán SM-2 (interval × hệ số dễ nhớ).
            </p>
          </div>
        </div>
      )}
      ...
      {/* Danger */}
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-red-600">⚠️ Nguy hiểm</h3>
      ...
      {/* Feedback */}
      {msg && (
        <div className={`rounded-lg px-3 py-2 text-xs ${msg.ok ? 'bg-accent-soft text-accent-dark' : 'bg-red-50 text-red-700'}`}>
          {msg.ok ? '✓' : '✗'} {msg.text}
        </div>
      )}
    </div>
    ...
    <div className="min-h-screen bg-[radial-gradient(circle_at_0%_0%,#e6f2eb,transparent_45%),linear-gradient(180deg,#f6f7f4,#e8ebe3)] p-3">
      ...
      {tab === 'list' && (
        <div>
          ...
          <h2 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-600">Danh sách từ</h2>
          {loading ? (
